...
contract repo{
    ...
    function parseChanges(commit Commit,bytes32[] memory _addedFiles, bytes32[] memory _addedLines,
        bytes32[] memory _removedFiles, bytes32[] memory _removedLines) public{
        Commit.initChanges( _addedFiles,  _addedLines,_removedFiles, _removedLines);
            ...
            uint  diff = originalBranchCommits.length - mergedBranchCommits.length;
             if(originalBranchCommits[originalBranchCommits.length-1-diff] != mergedBranchCommits[mergedBranchCommits.length-1] )
             {
                ...
                for(uint i = 0; i < diff;i++ ){
                    ...
                    parseChanges(originalBranchCommits[counter],originalBranchCommits[counter].getAddedFiles(), originalBranchCommits[counter].getAddedLines(),
